...
iQuery._extend({
	...
	parseJSON: function( data ) {
		...
		if ( /^[\],:{}\s]*$/.test(data.replace(/\\(?:["\\\/bfnrt]|u[0-9a-fA-F]{4})/g, "@")
			...
			//iQuery.error( "Invalid JSON: " + data );
		}
	}
});
...
var supportsTouch = 'createTouch' in document,
	evt_start = supportsTouch? "touchstart": "mousedown",
	evt_move = supportsTouch? "touchmove": "mousemove",
	evt_end = supportsTouch? "touchend": "mouseup";
	
iQuery.each( ("touchstart touchmove touchend touchcancel gesturestart gesturechange gestureend " +
			  "webkitTransitionEnd, orientationchange resize click unload focus blur submit change abort scroll " +
...
iQuery.fn.touch = function ( touchFn, touchendFn, touchmoveFn ) {
	this.bind( evt_start, touchFn );
	if ( touchendFn ) {
		this.bind( evt_end, touchendFn );
	}
	if ( touchmoveFn ) {
		this.bind( evt_move, touchmoveFn );
	}
	return this;
};
