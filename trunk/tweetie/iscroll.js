...
//		this.moved = true;
		...
		if (newX > this.options.edge.x || newX < this.maxScrollX) { 
			newX = this.options.bounce ? Math.round(this.x + leftDelta / 1.5) : newX >= 0 ? 0 : this.maxScrollX;
		}
		if (newY > this.options.edge.y || newY < this.maxScrollY) { 
			newY = this.options.bounce ? Math.round(this.y + topDelta / 1.5) : newY >= 0 ? 0 : this.maxScrollY;
		}
